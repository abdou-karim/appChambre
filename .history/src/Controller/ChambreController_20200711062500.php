...
class ChambreController extends AbstractController
{
 ...
 /**
  ...
  /**
     ...
     *  @param ChambreRepository $etudiantRepository
     *@Route("/deltechambre",options={"expose"=true}, name="supprimechambre")
     */
...

    {
        ...
        if($request->isXmlHttpRequest())
        {
            ...
            $statut=0; 
            $em = $this->getDoctrine()->getRepository('AppBundle:chambre')->find($iddelete);
            $em->setStatut($statut);
            $form=$this->createFormBuilder($em);
            $form->$form->handleRequest($request);
            $moi=$this->getDoctrine()->getManager();
            $em=$moi->getRepository('AppBundle:chambre')->find($iddelete);

          $em->flush();

         $this->addFlash('message','supprimer');
